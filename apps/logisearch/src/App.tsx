...
function App() {
  ...
  const handleSearch = useCallback(async (e?: React.KeyboardEvent | React.MouseEvent) => {
    ...
    try {
      if (searchMode === 'expert' || searchMode === 'general') {
        ...
        setHasSearched(true)
        saveSearch({ origin: 'N/A', destination: 'N/A', transport_mode: searchMode as any, results: { expertData: expertResult } }).then(saved => { if (saved?.id) setLastSearchId(saved.id) }).catch(() => {})
      } else {
  ...
  const handleSaveRFQ = useCallback(async () => {
    ...
    try { await saveRFQ({ search_query_id: lastSearchId, origin: p.origin, destination: p.destination, transport_mode: p.mode, cargo_details: query, status: 'pending' }); setSuccessMsg('RFQ guardado') }
    ...
    <Box sx={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <AppBar position="sticky">
        <Toolbar sx={{ maxWidth: 1200, width: '100%', mx: 'auto', px: { xs: 2, md: 3 } }}>
          <Box onClick={handleHome} sx={{ width: 40, height: 40, borderRadius: 2.5, background: 'linear-gradient(135deg, #fca5a5, #0096C7)', display: 'flex', alignItems: 'center', justifyContent: 'center', mr: 1.5, cursor: 'pointer' }}>
