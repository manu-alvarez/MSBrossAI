      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {TOOLS.map((tool) => (
          <a
            ...
            className="block p-5 bg-white dark:bg-[#1e1e1e] border border-[#EDEBE9] dark:border-[#333] shadow-sm hover:border-[#005A9E] dark:hover:border-[#4a9eff] transition-colors group"
          >
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-bold text-sm text-[#323130] dark:text-[#e0e0e0] group-hover:text-[#005A9E] dark:group-hover:text-[#4a9eff] transition-colors">
                {tool.name}
              </h3>
              <span className="text-[10px] font-semibold text-[#005A9E] dark:text-[#4a9eff] bg-[#F0F7FF] dark:bg-[#1a2a3a] px-2 py-0.5 rounded-full">
                msbross.me
              </span>
            </div>
            <p className="text-xs text-[#605E5C] dark:text-[#888]">{tool.description}</p>
            <div className="mt-3 text-xs font-semibold text-[#005A9E] dark:text-[#4a9eff] group-hover:underline">
              Abrir {tool.name} →
            </div>
          </a>
        ))}
