          ...
          <div key={s} className="flex items-center flex-1 last:flex-none">
            <button
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center font-kid-title text-base font-black transition-all ${
                s === step
                  ? "bg-gradient-to-r from-yellow-400 to-pink-500 text-white shadow-[0_0_15px_rgba(251,191,36,0.6)] scale-110"
                  : s < step
                  ? "bg-purple-500 text-white cursor-pointer hover:bg-purple-400"
            ...
            {s < 5 && (
              <div className={`flex-1 h-1.5 mx-2 rounded-full ${s < step ? "bg-purple-500" : "bg-white/10"}`} />
            )}
          </div>
        ))}
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {step === 1 && (
          <div className="space-y-6 fade-in-up">
            <h2 className="text-xl font-black font-kid-title text-purple-200 mb-4 flex items-center gap-2">
              <span>🚀</span> Misión 1: ¿Quién es el Héroe?
            </h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <label className="flex flex-col gap-2 text-sm">
                <span className="text-purple-200 font-bold text-base">Nombre del niño o niña</span>
                <input
                  className="px-5 py-4 rounded-2xl bg-purple-950/60 border-3 border-purple-500/40 focus:outline-none focus:border-purple-400 focus:ring-4 focus:ring-purple-400/20 text-white text-base placeholder-purple-400/40 transition font-bold"
                  value={childName}
                  required
              ...
              <label className="flex flex-col gap-2 text-sm">
                <span className="text-purple-200 font-bold text-base">Edad</span>
                <div className="relative">
                  <select
                    className="w-full px-5 py-4 rounded-2xl bg-purple-950/60 border-3 border-purple-500/40 focus:outline-none focus:border-purple-400 focus:ring-4 focus:ring-purple-400/20 text-white font-bold text-base cursor-pointer appearance-none"
                    value={childAge}
                    onChange={(e) => setChildAge(Number(e.target.value))}
                  >
                    {Array.from({ length: 15 }, (_, i) => i + 2).map((age) => (
                      <option key={age} value={age} className="bg-purple-950 text-white font-bold">
                        {age} años
                      </option>
                    ))}
                  </select>
            ...
            <div className="space-y-3">
              <span className="text-purple-200 font-bold text-base block">¡Elige tu Avatar de Héroe!</span>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-purple-200/60 leading-none mt-0.5">{av.desc}</div>
                      </div>
                    </button>
                  );
                })}
        ...
        {step === 2 && (
          <div className="space-y-6 fade-in-up">
            <h2 className="text-xl font-black font-kid-title text-pink-300 mb-4 flex items-center gap-2">
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl border-3 p-4 transition flex flex-col items-center justify-center text-center cursor-pointer h-28 ${
                      active
                        ? "bg-pink-500/20 border-pink-400 shadow-[0_0_20px_rgba(244,63,94,0.4)] scale-105"
            ...
            {theme === "Otra aventura..." && (
              <div className="space-y-2 fade-in-up">
                <label className="flex flex-col gap-1 text-sm">
                  <span className="text-purple-200 font-bold">Describe tu propia aventura mágica</span>
                  <input
                    className="px-5 py-4 rounded-2xl bg-purple-950/60 border-3 border-purple-500/40 focus:outline-none focus:border-purple-400 focus:ring-4 focus:ring-purple-400/20 text-white font-bold"
                    value={customTheme}
                    required
        ...
        {step === 3 && (
          <div className="space-y-6 fade-in-up">
            ...
            <p className="text-purple-200/80 text-sm font-semibold mb-4">
          ...
          <div className="space-y-6 fade-in-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
              <div className="flex flex-col gap-2">
                <span className="text-purple-200 font-bold text-base">Duración aproximada</span>
                    ...
                    <button
                      ...
                      className={`flex-1 py-3 rounded-2xl border-3 transition font-extrabold cursor-pointer text-xs md:text-sm ${
                        duration === opt.id
                          ...
                          : "bg-pink-950/40 text-pink-100 border-pink-500/30 hover:border-pink-300"
                      }`}
                    >
                      {opt.label}
                    </button>
              ...
              <div className="flex flex-col gap-2">
                <span className="text-purple-200 font-bold text-base">Idioma del Reino</span>
                <div className="flex gap-3">
                  {[
                    ...
                    <button
                      ...
                      className={`flex-1 py-3 rounded-2xl border-3 transition font-extrabold cursor-pointer text-xs md:text-sm ${
                        language === opt.id
                          ...
                          : "bg-emerald-950/40 text-emerald-100 border-emerald-500/30 hover:border-emerald-300"
                      }`}
                    >
                      {opt.label}
                    </button>
        ...
        {step === 5 && (
          <div className="space-y-6 fade-in-up">
            <h2 className="text-xl font-black font-kid-title text-indigo-300 mb-4 flex items-center gap-2">
              <span>🎬</span> Misión 5: Formato de la Magia
            </h2>
            
            <div className="flex flex-col gap-3 text-sm">
              <span className="text-purple-200 font-bold text-base">Tipo de Contenido Mágico</span>
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-2xl border-3 p-4 text-left transition cursor-pointer flex flex-col justify-between h-32 ${
                        active
                          ? "bg-indigo-500/20 border-indigo-400 shadow-[0_0_15px_rgba(99,102,241,0.3)] scale-102"
                          : "bg-white/5 border-white/10 opacity-70 hover:opacity-100"
                      }`}
                    >
                      <div>
                      ...
                      <div className="text-[10px] text-purple-200/50 leading-tight mt-1">{ct.desc}</div>
                    </button>
                  );
                })}
              </div>
        ...
        {/* Navigation actions */}
        <div className="flex items-center justify-between gap-4 border-t border-purple-500/20 pt-6">
          {step > 1 ? (
            <button
              type="button"
              onClick={handlePrevStep}
              className="px-6 py-3.5 rounded-2xl border-3 border-purple-500/30 bg-purple-950/40 font-black text-sm md:text-base cursor-pointer hover:bg-purple-950/80 transition"
            >
              ← Misión Anterior
            </button>
          ) : (
          ...
          {step < 5 ? (
            <button
              ...
              className="px-8 py-3.5 rounded-2xl bg-gradient-to-r from-purple-500 to-pink-500 font-black text-sm md:text-base shadow-lg cursor-pointer hover:brightness-110 active:scale-95 transition"
            >
              Siguiente Misión →
            </button>
          ) : (
            <button
              type="submit"
              disabled={loading}
              className="magic-glow px-10 py-4 rounded-2xl bg-gradient-to-r from-yellow-400 via-pink-500 to-indigo-600 font-black text-base md:text-lg shadow-[0_0_30px_rgba(251,191,36,0.6)] hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed transition cursor-pointer select-none"
            >
              {loading ? (
