...
def update_timer(tid: int, data: dict, user: dict = Depends(get_current_user)):
...
def update_valve(vid: int, data: dict, user: dict = Depends(get_current_user)):
    return db.update_valve(vid, data)

@app.post("/api/operations/{oid}/valves")
def create_valve(oid: int, data: dict, user: dict = Depends(get_current_user)):
    return db.create_valve(oid, data.get("name", "New Valve"))

# --- Pumps ---
@app.put("/api/pumps/{pid}")
def update_pump(pid: int, data: dict, user: dict = Depends(get_current_user)):
    return db.update_pump(pid, data)

@app.post("/api/operations/{oid}/pumps")
def create_pump(oid: int, data: dict, user: dict = Depends(get_current_user)):
    return db.create_pump(oid, data.get("name", "New Pump"))

# --- Checklists ---
@app.put("/api/checklists/{cid}")
def update_checklist(cid: int, data: dict, user: dict = Depends(get_current_user)):
    return db.update_checklist_item(cid, data)

@app.post("/api/operations/{oid}/checklists")
def create_checklist(oid: int, data: dict, user: dict = Depends(get_current_user)):
