        ...
        {TEMARIO.map((mod: any, i: number) => {
          ...
          return (
            <motion.div 
              ...
              className={`glass-panel rounded-2xl overflow-hidden group transition-all duration-300 ${isOpen ? 'ring-2 ring-neon-cyan/50 shadow-[0_0_30px_rgba(0,255,204,0.15)]' : ''}`}
            >
              <div 
              ...
              >
                <button 
                  ...
                  className={`mt-1 flex-shrink-0 w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${isDone ? 'bg-neon-cyan border-neon-cyan text-deep-void shadow-[0_0_15px_rgba(0,255,204,0.6)]' : 'border-muted text-transparent group-hover:border-neon-cyan/50'}`}
                >
                  <CheckCircle size={20} className={isDone ? 'opacity-100' : 'opacity-0'} />
                </button>
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-xs font-bold font-mono px-3 py-1 rounded-md bg-neon-cyan/10 border border-neon-cyan/30 text-neon-cyan shadow-[0_0_10px_rgba(0,255,204,0.2)]">{mod.code}</span>
                    <span className="text-xs font-mono text-muted bg-surface border border-border px-3 py-1 rounded-md">{mod.from} → {mod.to}</span>
                  </div>
                  <h3 className="text-xl font-extrabold text-white group-hover:text-neon-cyan transition-colors">{mod.title}</h3>
                </div>
                <ChevronDown className={`text-muted transition-transform duration-300 mt-2 ${isOpen ? 'rotate-180 text-neon-cyan' : ''}`} size={24} />
              </div>
                    ...
                    <div className="p-6 space-y-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="bg-surface2/50 border border-border/50 rounded-2xl p-5">
                          ...
                          <ul className="space-y-3">
                            {mod.gr?.map((t:string, j:number) => <li key={j} className="text-sm text-gray-300 flex items-start gap-3"><span className="text-neon-cyan mt-0.5">›</span> {t}</li>)}
                          </ul>
                        </div>
                        <div className="bg-surface2/50 border border-border/50 rounded-2xl p-5">
                          ...
                          <div className="space-y-3">
                            {mod.ph?.map((p:string, j:number) => (
                              <div key={j} className="text-sm font-mono bg-[#02050A]/80 border border-border/30 rounded-xl p-3 text-neon-cyan leading-relaxed shadow-inner">"{p}"</div>
                            ))}
                          </div>
                        </div>
                      </div>
